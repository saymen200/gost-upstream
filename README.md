...
В режиме `--ssh-target` при старте сразу проверяется, отвечает ли VM по
SSH — если нет, печатается явное предупреждение (а не молчаливые
непонятные ошибки на каждый запрос позже). Прокси при этом всё равно
запускается — обычный TLS продолжит работать, ГОСТ-фолбэк просто не
сработает, пока VM не поднимется.
...
- Маршрут (обычный TLS / ГОСТ) кэшируется по `host:port` — двойную попытку
  (обычный TLS, потом ГОСТ) платит только первый запрос к хосту, дальше
  прокси сразу идёт по уже известному пути. Если закешированный маршрут
  вдруг перестаёт работать (сеть, смена конфигурации хоста) — не падаем
  на кэше слепо, перепробуем оба варианта заново. Кэш живёт на время
  работы процесса, без TTL.
- Для генуинно недоступных хостов (не резолвится, не отвечает) каждая
  такая перепроверка платит двойную попытку: обычный TLS + до ~10 секунд
  ожидания первого байта от ГОСТ-цели, прежде чем вернуть ошибку.
